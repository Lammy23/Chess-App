...
export function possiblePawnMoves({ futureBoardState, teamColour }) {
	...
	const check = (coordinate) => {
		...
		if (coordinate.rank === specialRank) {
			if (
				coordinate.plus({ fileStep: 0, rankStep: 2 * pawnDirection }) !==
				origin /* also check that no enemy piece is in the spot */
			) {
				if (!coordinate.isOccupied(futureBoardState)) {
					// DEBUG: Seeing which spaces are free
	...
	const pawnPromotionMoveMap = pawnPromotion({
		moveMap,
		futureBoardState,
		teamColour,
	});
	...
	const check = (coordinate) => {
		...
		if (
			...
			if (coordinate.isOccupiedByOpponent(futureBoardState, teamColour)) {
				// DEBUG: Seeing which spaces are occupied by opponent
				// console.log(coordinate.coordinate);
				moveList.push(coordinate.coordinate);
				moveMap.push({
					from: origin,
...
export function pawnPromotion({ moveMap, futureBoardState, teamColour }) {
	const isPromotionRank = (rank) =>
		teamColour === Color.white ? rank === 8 : rank === 1; // Determining the rank required for pawn promotion of a given team colour
