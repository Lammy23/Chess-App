...
export const MovementProvider = ({ children, appRef }) => {
	...
	const [lastMoveWasCastleKingSide, setLastMoveWasCastleKingSide] = useState(false);
	const [lastMoveWasCastleQueenSide, setLastMoveWasCastleQueenSide] = useState(false);
        ...
        from its starting position */
            ...
            This automatically triggers a re-render (as it's a state variable) */
						...
						if (referee.isCastle) {
							...
							const currentRookCoordinates = referee.castleMoveDetails.currentRookCoordinates
							const oldRookCoordinates = referee.castleMoveDetails.oldRookCoordinates
							updatedPosition[currentRookCoordinates] =
								prev[oldRookCoordinates];
	...
	useEffect(() => {
		// If any of the pieces move, change the flags accordingly
		if (boardState) {
